...
+ states that it has two constructors of the form:
	+ `[]`, or
...
```
this defines a new data type with name `Fraction` and constructor with the form `Fraction n d`, where n and d are both integers. here, the constructor is a function, and must take in tcanwo `Integer`s. in fact, even these literals from before are also functions!
> [!TIP]
...
```
because of our type definition, we can easily use it again here to pattern match the constructor, just like we pattern match the two [[haskell lists|list]] constructors for lists.
## subclassing types
